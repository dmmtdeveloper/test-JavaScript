...
export const getTipoCuartel = async (req, res) => {
  try {
    const { rows, rowCount } = await db.query(
      `SELECT tc.name, ROUND(SUM(qs.area/10000),3) AS hectarea 
        FROM tipo_cuartel tc 
        JOIN cuarteles qs ON id_paddock_type = tc.id GROUP BY tc.id, tc.name 
        ORDER BY hectarea DESC`
    );
...
export const getAdminForestalAgricola = async (req, res) => {
  try {
    const { rows, rowCount } = await db.query(
      `SELECT a.name, SUM(c.area) AS area_total_cerezas 
        FROM administradores a 
        JOIN cuarteles c ON a.id = c.id_admin 
        JOIN granjas g ON c.id_farm = g.id 
        JOIN tipo_cuartel tc ON c.id_paddock_type = tc.id 
        WHERE g.name = 'FORESTAL Y AGRICOLA LO ENCINA' 
        AND tc.name = 'CEREZAS' 
        AND c.area > 1000 
...
export const getYearCultivo = async (req, res) => {
  try {
    const { rows, rowCount } = await db.query(
      `SELECT CONCAT(tc.name, '-', c.harvest_year) AS tipo_de_cultivo, json_object_agg(a.id, a.name) AS administradores 
      FROM cuarteles c 
      JOIN tipo_cuartel tc ON c.id_paddock_type = tc.id 
      JOIN administradores a ON c.id_admin = a.id 
      GROUP BY tc.name, c.harvest_year;`
    );
    console.log("Tipo de cultivo año mas administrador", rows);
    if (!rows.length)
      return res
        .status(404)
        .json({ message: "Error al intentar ontener los datos" });
    res.status(200).json({
      tipo: rows,
      count: rowCount,
